...
Bot.prototype.disconnect=function(){
 ...
 if(ws){try{ws.removeAllListeners();if(ws.readyState===WebSocket.CONNECTING)ws.terminate();else if(ws.readyState===WebSocket.OPEN||ws.readyState===WebSocket.CLOSING)ws.close();}catch(_){} }
...
Bot.prototype.onMessage=function(data){const b=Buffer.from(data);if(b.length<3)return;const type=String.fromCharCode(b[2]);this.lastPacketType=type;this.lastPacketLength=b.length;if(type==='6'){try{this.send(this.decodeSecret(b));}catch(e){this.lastError=`Challenge decode failed: ${e.message}`;try{if(this.ws&&this.ws.readyState===WebSocket.OPEN)this.ws.close(1002,'challenge');else if(this.ws&&this.ws.readyState===WebSocket.CONNECTING)this.ws.terminate();}catch(_){}return;}this.state='verified';this.spawn();return;}if(type==='p'){this._pingInFlight=false;return;}if(type==='a'){this.serverProtocolVersion=b.length>25?b[25]:null;this.startPingLoop();if(this.joined)this.startMovementLoop();return;}if(type==='v'){this.joined=false;this.state='dead';clearTimeout(this._movementTimer);this._movementTimer=null;return;}if((type==='g'||type==='n')&&b.length>=9){const id=b.readUInt16BE(3);if(id===this.snakeID){this.snakeX=b.readInt16BE(5);this.snakeY=b.readInt16BE(7);}return;}if((type==='G'||type==='N')&&b.length>=7&&this.haveSnakeID){const id=b.readUInt16BE(3);if(id===this.snakeID){this.snakeX+=b.readInt8(5)-128;this.snakeY+=b.readInt8(6)-128;}return;}if(type==='s'&&b.length>=6){const id=b.readUInt16BE(3);if(!this.haveSnakeID&&b.length>=34){this.snakeID=id;this.haveSnakeID=true;this.joined=true;this.state='joined';this.snakeX=readU24(b.subarray(18,21))/5;this.snakeY=readU24(b.subarray(21,24))/5;if(this.botManager){this.botManager.sendCountUpdate();this.startMovementLoop();}}else if(this.haveSnakeID&&id===this.snakeID&&b.length===6){this.haveSnakeID=false;this.joined=false;this.state='dead';}}};
module.exports=Bot;
