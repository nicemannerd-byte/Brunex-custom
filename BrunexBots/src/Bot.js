...
function Bot(id){
 ...
 this.targetX=null;this.targetY=null;this.snakeID=null;this.snakeX=0;this.snakeY=0;this.snakes=new Map();this.autoTargetId=null;
...
Bot.prototype.rememberSnake=function(id,x,y){if(!Number.isFinite(Number(x))||!Number.isFinite(Number(y)))return;const now=Date.now(),old=this.snakes.get(id);const vx=old?Number(x)-old.x:0,vy=old?Number(y)-old.y:0;this.snakes.set(id,{id,x:Number(x),y:Number(y),vx,vy,seenAt:now});};
Bot.prototype.pruneSnakes=function(){const cutoff=Date.now()-5000;for(const [id,snake] of this.snakes)if(snake.seenAt<cutoff||id===this.snakeID)this.snakes.delete(id);};
Bot.prototype.getHeadHunterTarget=function(){this.pruneSnakes();if(this.targetX!==null&&this.targetY!==null)return{x:this.targetX,y:this.targetY,manual:true};let best=null;for(const snake of this.snakes.values()){const distance=Math.hypot(snake.x-this.snakeX,snake.y-this.snakeY);if(!best||distance<best.distance)best={...snake,distance};}if(best&&best.id!==this.autoTargetId){this.autoTargetId=best.id;this.log(`auto-targeting visible snake ${best.id} at ${Math.round(best.distance)} units`);}return best;};
Bot.prototype.getHeadHunterSteering=function(){const target=this.getHeadHunterTarget();if(!target)return null;const dx=target.x-this.snakeX,dy=target.y-this.snakeY,distance=Math.hypot(dx,dy)||1;let closest=null;for(const snake of this.snakes.values()){const d=Math.hypot(snake.x-this.snakeX,snake.y-this.snakeY);if(!closest||d<closest.distance)closest={...snake,distance:d};}
 // Preserve a safety margin around any observed enemy head. The existing parser
 // supplies head coordinates only; body geometry is deliberately not invented.
 if(closest&&closest.distance<650){const away=Math.atan2(this.snakeY-closest.y,this.snakeX-closest.x)+(this.id%2?0.9:-0.9);return{x:this.snakeX+Math.cos(away)*2000,y:this.snakeY+Math.sin(away)*2000,boost:false};}
 const velocity=Math.hypot(target.vx||0,target.vy||0),lead=Math.min(900,Math.max(250,distance*.22));const ux=velocity?(target.vx/velocity):(dx/distance),uy=velocity?(target.vy/velocity):(dy/distance);const side=this.id%2?1:-1;const lateral=Math.min(500,Math.max(150,distance*.12));return{x:target.x+ux*lead-uy*lateral*side,y:target.y+uy*lead+ux*lateral*side,boost:!!this.persistentBoost&&distance>500&&distance<5000};};
Bot.prototype.setBoost=function(enabled){const next=!!enabled;if(this.isBoost===next)return;this.isBoost=next;this.send(Buffer.from([next?253:254]));};
Bot.prototype.moveTo=function(x,y){if(!Number.isFinite(Number(x))||!Number.isFinite(Number(y)))return;let tx=Number(x),ty=Number(y);if((this.mode==='headhunter'||this.mode==='focus')&&this.targetX!==null&&this.targetY!==null){tx=this.targetX;ty=this.targetY;}let a=Math.atan2(ty-this.snakeY,tx-this.snakeX);if(this.mode==='wander')a+=((this.id%7)-3)*.35;if(this.tornadoActive)a+=Math.PI/2;if(this.fixedCircleActive)a+=(this.id%2?1:-1)*Math.PI/3;if(this.mode==='focus')a+=(this.id%2?1:-1)*.2;while(a<0)a+=Math.PI*2;while(a>=Math.PI*2)a-=Math.PI*2;const v=Math.max(0,Math.min(250,Math.floor(a*125/Math.PI)));this.send(Buffer.from([v]));};
...
Bot.prototype.boostSpeed=function(v){this.persistentBoost=!!v;if(this.mode!=='headhunter')this.setBoost(this.persistentBoost);};
Bot.prototype.startMovementLoop=function(){clearTimeout(this._movementTimer);const loop=()=>{if(!this.botManager||!this.botManager.isRunning()||!this.joined)return;const tactical=this.mode==='headhunter'?this.getHeadHunterSteering():null;const t=tactical||this.botManager.getMovementTarget();if(t&&Number.isFinite(t.x)&&Number.isFinite(t.y)){if(tactical)this.setBoost(tactical.boost);this.moveTo(t.x,t.y);}this._movementTimer=setTimeout(loop,180);};loop();};
