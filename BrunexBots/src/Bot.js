...
// Protocol 19 is the newest public Slither.io web protocol implementation
// identified in Vlither (May 2026). Keep an environment override for private
// servers that intentionally expose another protocol.
...
function Bot(id){
 ...
 this.lastError='';this.lastUrl='';this.lastClose='';this.lastHttpStatus=null;this.attempt=0;
...
Bot.prototype.connect=function(){
 ...
 try{options.headers.Host=new URL(url).host;}catch(e){this.lastError=`Invalid server endpoint: ${e.message}`;this.state='reconnecting';this.scheduleReconnect(500);return;}
 try{
  this.ws=new WebSocket(url,options);this.ws.binaryType='nodebuffer';
  this.ws.on('message',d=>this.onMessage(d));this.ws.on('open',()=>this.onOpen());
  this.ws.on('close',(code,reason)=>this.onClose(code,reason));this.ws.on('error',e=>this.onError(e));
  this.ws.on('unexpected-response',(req,res)=>{this.lastHttpStatus=res.statusCode;this.lastError=`HTTP ${res.statusCode}`;});
...
Bot.prototype.buildSpawnPacket=function(){
 const name=this.name.slice(0,24);const bytes=Buffer.from(name,'utf8');
 const p=Buffer.alloc(4+bytes.length);p[0]=115;p[1]=Math.max(0,Math.min(255,PROTOCOL_VERSION-1));
 p[2]=Math.max(0,Math.min(38,Number(this.cosmetic)||0));p[3]=bytes.length;bytes.copy(p,4);return p;
};
...
Bot.prototype.onOpen=function(){
 this.state='connected';this.hasConnected=true;this.joined=false;this.lastError='';
...
Bot.prototype.onClose=function(code,reason){
 clearTimeout(this._movementTimer);clearInterval(this._pingTimer);this._movementTimer=this._pingTimer=null;this._pingInFlight=false;
 this.lastClose=`${code}${reason&&reason.length?' '+Buffer.from(reason).toString('utf8'):''}`;
 if(this.hasConnected&&this.botManager){this.hasConnected=false;this.botManager.setConnectedCount(Math.max(0,this.botManager.getConnectedCount()-1));this.botManager.sendCountUpdate();}
...
Bot.prototype.onError=function(e){this.lastError=e&&e.message||String(e);};

// The public protocol-11 documentation describes the challenge transformation.
// Protocol 19 clients still require the server's current verification response;
// retain this decoder as a compatibility path while allowing the server to
// reject/close cleanly if its challenge format has changed.
Bot.prototype.decodeSecret=function(secret){
 if(!secret||secret.length<65)throw new Error(`invalid challenge length ${secret?secret.length:0}`);
...
Bot.prototype.startMovementLoop=function(){
 clearTimeout(this._movementTimer);
 const loop=()=>{
  if(!this.botManager||!this.botManager.isRunning()||!this.joined)return;
  const t=this.botManager.getMovementTarget();
  if(t&&Number.isFinite(t.x)&&Number.isFinite(t.y))this.moveTo(t.x,t.y);
  this._movementTimer=setTimeout(loop,180);
 };
 loop();
};
Bot.prototype.startPingLoop=function(){
 clearInterval(this._pingTimer);
 this._pingTimer=setInterval(()=>{if(!this._pingInFlight){this._pingInFlight=true;this.send(Buffer.from([251]));}},250);
};

Bot.prototype.onMessage=function(data){
 const b=Buffer.from(data);if(b.length<3)return;
 const type=String.fromCharCode(b[2]);this.lastPacketType=type;this.lastPacketLength=b.length;

 if(type==='6'){
  try{this.send(this.decodeSecret(b));}
  catch(e){this.lastError=`Challenge decode failed: ${e.message}`;try{this.ws.close(1002,'challenge');}catch(_){}return;}
  this.state='verified';this.spawn();return;
 }
 if(type==='p'){this._pingInFlight=false;return;}
 if(type==='a'){
  this.serverProtocolVersion=b.length>25?b[25]:null;
  this.startPingLoop();if(this.joined)this.startMovementLoop();return;
 }
 if(type==='v'){this.joined=false;this.state='dead';clearTimeout(this._movementTimer);this._movementTimer=null;return;}

 if((type==='g'||type==='n')&&b.length>=9){
  const id=b.readUInt16BE(3);if(id===this.snakeID){this.snakeX=b.readInt16BE(5);this.snakeY=b.readInt16BE(7);}return;
 }
 if((type==='G'||type==='N')&&b.length>=7&&this.haveSnakeID){
  const id=b.readUInt16BE(3);if(id===this.snakeID){this.snakeX+=b.readInt8(5)-128;this.snakeY+=b.readInt8(6)-128;}return;
 }
 if(type==='s'&&b.length>=6){
  ...
  if(!this.haveSnakeID&&b.length>=34){
   ...
   if(this.botManager){this.botManager.sendCountUpdate();this.startMovementLoop();}
  }else if(this.haveSnakeID&&id===this.snakeID&&b.length===6){
   this.haveSnakeID=false;this.joined=false;this.state='dead';
  }
 }
};
