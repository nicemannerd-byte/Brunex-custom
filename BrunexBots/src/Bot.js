...
// The documented Slither.io client uses http://slither.io as the Origin.
const DEFAULT_ORIGIN=process.env.GAME_ORIGIN||'http://slither.io';

function Bot(id){
 this.id=id;this.server='';this.serverInput='';this.origin=DEFAULT_ORIGIN;this.botManager=null;this.cosmetic=11;this.tag=12;this.persistentBoost=false;this.mode='follow';
 this.targetX=null;this.targetY=null;this.snakeID=null;this.snakeX=0;this.snakeY=0;this.haveSnakeID=false;this.hasConnected=false;this.joined=false;this.isBoost=false;
 this._reconnectTimer=null;this._movementTimer=null;this._pingTimer=null;this._manualDisconnect=false;this._candidateIndex=-1;this._pingInFlight=false;this.state='connecting';this.name='';
 this.lastError='';this.lastUrl='';this.lastClose='';this.lastHttpStatus=null;this.attempt=0;this.serverProtocolVersion=null;this.lastPacketType='';this.lastPacketLength=0;
 this.tornadoSize=2000;this.tornadoSeparation=800;this.tornadoFinalAngle=90;this.tornadoRateOfChange=.5;this.tornadoRotationDir=1;this.tornadoActive=false;this.fixedCircleActive=false;
}
...
Bot.prototype.connectionCandidates=function(){
 ...
 if(hasPath)return[`ws://${host}`,`wss://${host}`];
 // Protocol documentation and the original client use ws://<ip>:<port>/slither.
 // Keep wss as a fallback for servers explicitly terminating TLS.
 if(port==='443')return[`ws://${host}/slither`,`ws://${host}/`,`wss://${host}/slither`,`wss://${host}/`];
...
Bot.prototype.connect=function(){
 ...
 this._candidateIndex=(this._candidateIndex+1)%urls.length;const url=urls[this._candidateIndex];this.lastUrl=url;this.lastHttpStatus=null;this.attempt++;
 ...
 try{const u=new URL(url);options.headers.Host=u.host;}
 catch(e){this.lastError=`Invalid server endpoint: ${e.message}`;this.state='reconnecting';this.scheduleReconnect(500);return;}
 try{
  ...
  this.ws.on('unexpected-response',(req,res)=>{this.lastHttpStatus=res.statusCode;this.lastError=`HTTP ${res.statusCode}`;if(process.env.DEBUG==='1')console.error(`[Bot ${this.id}] ${url} -> HTTP ${res.statusCode}`);});
...
Bot.prototype.disconnect=function(){this._manualDisconnect=true;clearTimeout(this._reconnectTimer);clearTimeout(this._movementTimer);clearInterval(this._pingTimer);this._reconnectTimer=this._movementTimer=this._pingTimer=null;this._pingInFlight=false;if(this.ws){try{this.ws.removeAllListeners();this.ws.close();}catch(_){}this.ws=null;}if(this.hasConnected&&this.botManager){this.hasConnected=false;this.botManager.setConnectedCount(Math.max(0,this.botManager.getConnectedCount()-1));}this.state='stopped';this.joined=false;this.haveSnakeID=false;};
Bot.prototype.onOpen=function(){this.state='connected';this.hasConnected=true;this.joined=false;this.lastError='';this.send(Buffer.from([99]));if(this.botManager){this.botManager.setConnectedCount(this.botManager.getConnectedCount()+1);this.botManager.sendCountUpdate();}};
Bot.prototype.onClose=function(code,reason){clearTimeout(this._movementTimer);clearInterval(this._pingTimer);this._movementTimer=this._pingTimer=null;this._pingInFlight=false;this.lastClose=`${code}${reason&&reason.length?' '+Buffer.from(reason).toString('utf8'):''}`;if(this.hasConnected&&this.botManager){this.hasConnected=false;this.botManager.setConnectedCount(Math.max(0,this.botManager.getConnectedCount()-1));this.botManager.sendCountUpdate();}this.joined=false;this.haveSnakeID=false;this.ws=null;if(!this._manualDisconnect&&this.botManager&&this.botManager.isRunning()){this.state='reconnecting';this.scheduleReconnect();}else this.state='stopped';};
...
Bot.prototype.decodeSecrect=function(secret){if(!secret||secret.length<65)throw new Error(`invalid challenge length ${secret?secret.length:0}`);const out=new Uint8Array(24);let state=0;for(let i=0;i<24;i++){let a=secret[17+i*2];if(a<=96)a+=32;a=(a-98-i*34)%26;if(a<0)a+=26;let b=secret[18+i*2];if(b<=96)b+=32;b=(b-115-i*34)%26;if(b<0)b+=26;let v=(a<<4)|b;const base=v>=97?97:65;v-=base;if(i===0)state=2+v;out[i]=((v+state)%26)+base;state+=3+v;}return Buffer.from(out);};
...
Bot.prototype.onMessage=function(data){
 const b=Buffer.from(data);if(b.length<3)return;const type=String.fromCharCode(b[2]);this.lastPacketType=type;this.lastPacketLength=b.length;
 if(type==='6'){
  ...
  this.state='verified';this.spawn();
 } else if(type==='p'){
  this._pingInFlight=false;
 } else if(type==='a'){
  // Protocol v11 stores protocol_version at byte 25.
  this.serverProtocolVersion=b.length>25?b[25]:null;
  clearTimeout(this._movementTimer);
  const loop=()=>{if(!this.botManager||!this.botManager.isRunning()||!this.joined)return;const t=this.botManager.getMovementTarget();if(t&&Number.isFinite(t.x)&&Number.isFinite(t.y))this.moveTo(t.x,t.y);this._movementTimer=setTimeout(loop,180);};
  if(this.joined)loop();
  ...
  const id=b.readUInt16BE(3);if(id===this.snakeID){this.snakeX=b.readInt16BE(5);this.snakeY=b.readInt16BE(7);}
 } else if((type==='G'||type==='N')&&b.length>=7&&this.haveSnakeID){
  const id=b.readUInt16BE(3);if(id===this.snakeID){this.snakeX+=b.readInt8(5)-128;this.snakeY+=b.readInt8(6)-128;}
 } else if(type==='s'&&b.length>=6){
  const id=b.readUInt16BE(3);
  if(!this.haveSnakeID&&b.length>=34){
   this.snakeID=id;this.haveSnakeID=true;this.joined=true;this.state='joined';
   this.snakeX=readU24(b.subarray(18,21))/5;this.snakeY=readU24(b.subarray(21,24))/5;
   if(this.botManager)this.botManager.sendCountUpdate();
  } else if(this.haveSnakeID&&id===this.snakeID&&b.length===6){this.haveSnakeID=false;this.joined=false;this.state='dead';}
 }
};
module.exports=Bot;
