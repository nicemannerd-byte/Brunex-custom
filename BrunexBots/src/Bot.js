...
Bot.prototype.startMovementLoop=function(){clearTimeout(this._movementTimer);const loop=()=>{if(!this.botManager||!this.botManager.isRunning()||!this.joined)return;const t=this.mode==='headhunter'?this.botManager.getHeadHunterTarget(this):this.botManager.getMovementTarget();if(t&&Number.isFinite(t.x)&&Number.isFinite(t.y)){this.moveTo(t.x,t.y);if(this.mode==='headhunter'){const shouldBoost=!!t.boost&&Number.isFinite(t.distance)&&t.distance>500&&t.distance<2000;this.boostSpeed(shouldBoost);}}this._movementTimer=setTimeout(loop,180);};loop();};
