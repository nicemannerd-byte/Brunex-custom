...
async function compatibleServerTest(){
 const server=http.createServer();const wss=new WebSocketServer({server});const port=await listen(server);let stage=0;const moves=[];
 wss.on('connection',ws=>ws.on('message',packet=>{const p=Buffer.from(packet);if(stage>=3){moves.push(p);return;}if(stage===0){assert.deepStrictEqual([...p],[99]);stage=1;const challenge=Buffer.alloc(65,65);challenge[2]=54;ws.send(challenge);return;}if(stage===1){assert.strictEqual(p.length,24);stage=2;return;}if(stage===2){assert.strictEqual(p[0],115);assert.strictEqual(p[1],18);stage=3;const spawn=Buffer.alloc(34);spawn[2]=115;spawn.writeUInt16BE(321,3);spawn[18]=0;spawn[19]=1;spawn[20]=244;spawn[21]=0;spawn[22]=3;spawn[23]=232;ws.send(spawn);const enemy=Buffer.from(spawn);enemy.writeUInt16BE(654,3);enemy[18]=0;enemy[19]=39;enemy[20]=16;enemy[21]=0;enemy[22]=58;enemy[23]=144;ws.send(enemy);}}));
 const bot=new Bot(1);bot.setMode('headhunter');bot.boostSpeed(true);bot.serverInput=`ws://127.0.0.1:${port}/slither`;bot.botManager=manager;bot.connect();await waitFor(()=>bot.joined);
 assert.strictEqual(bot.state,'joined');assert.strictEqual(bot.snakeID,321);assert.strictEqual(manager.connected,1);assert.strictEqual(stage,3);await waitFor(()=>bot.autoTargetId===654);await waitFor(()=>moves.some(p=>p[0]===253));assert.strictEqual(bot.snakes.get(654).x,2000);bot.disconnect();await close(wss);await close(server);
}
